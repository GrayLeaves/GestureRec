...
def step_train(train_op, loss, accuracy, train_images_batch, train_labels_batch, train_nums, train_log_step,
    ...
    :param train_op: 训练op
    :param loss:     loss函数
    :param accuracy: 准确率函�?    :param train_images_batch: 训练images数据
    :param train_labels_batch: 训练labels数据
    :param train_nums:         总训练数�?    :param train_log_step:     训练log显示间隔
    :param val_images_batch:   验证images数据
    :param val_labels_batch:   验证labels数据
    :param val_nums:           总验证数�?    :param val_log_step:       验证log显示间隔
    :param snapshot_prefix:    模型保存的路�?    :param snapshot:           模型保存间隔
    :return: None
    '''
...
def train(train_record_file, train_log_step, train_param, val_record_file,
    ...
    :param num_classes:       labels�?    :param data_shape:        输入数据shape
    :param snapshot:          保存模型间隔
    :param snapshot_prefix:   保存模型文件的前缀�?    :return:
    '''
    ...
    train_images, train_labels = read_records(train_record_file, resize_height, resize_width, type='normalization', is_train=None)
    ...
    # 使用`tensorflow`的控制流, 先执行更新算�? 再执行训�?    with tf.control_dependencies(update_ops):
